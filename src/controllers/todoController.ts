...
export const updateTodo = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const row = todo as TodoRow;
    const updatedTask = payload.task !== undefined ? payload.task : row.task;
    const updatedStatus = payload.is_completed !== undefined ? payload.is_completed : Boolean(row.is_completed);

    await TodoModel.update(Number(id), updatedTask, updatedStatus, userId);
    ...
    sendSuccess(res, 'Tugas berhasil diperbarui!', data);
  } catch {
